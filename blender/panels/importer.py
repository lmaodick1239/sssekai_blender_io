...
def _is_timeline_candidate(reader):
    """Identify Timeline track readers without trusting missing type metadata."""
    ...
    if callable(traverse):
        try:
            return any(
                getattr(field, "m_Name", None) == "m_Parent" for field in traverse()
            )
        except Exception:
            pass
